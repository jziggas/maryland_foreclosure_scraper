...
# For reference / possible expansion of code
COUNTY_CODES = { 
  "District Court" => 'DC',
...
county_code = '24'
case_code = 'O'
end_case_number = '004000'
start_case_number = '000001'
case_code_length = 6
title = ''
filing_date =''
property_addresses = []
...
year = gets.chomp
puts "Enter the six digit case number you want to start with (e.g. 134 would be '000134'): "
start_case_number = gets.chomp
puts "Enter the six digit case number you want to start with (e.g. 2765 would be '002765'): "
end_case_number = gets.chomp.to_i

# Data will be saved to "name_of_county_foreclosure_startcase-endcase_year.csv"
filename = COUNTY_CODES.invert[county_code].split[0..1].join('_').downcase + '_' + CASE_CODES.invert[case_code].downcase + "_#{start_case_number.to_i}-#{end_case_number}_" + "20#{year}" + '.csv'
...
CSV.open(filename, 'w', options) do |f|
  # Currently there's no implementation for checking when the end of a year's records occur. So the scraper will keep iterating case numbers up to end_case_code. The user must keep an eye on the command line window. When a case starts repeating itself, it's time to CTRL+C end the script.
  while start_case_number.to_i <= end_case_number
    ...
    case_search = county_code + case_code + year + start_case_number
    ...
    puts "Case number: #{start_case_number}\t\tTitle: #{title}"
